...
namespace PBS.Player
{
    public class BattleControls : MonoBehaviour
    {
        ...
        private PBS.Battle.View.Compact.Pokemon commandPokemon;
        ...
        private IEnumerator ControlPromptCommand(
            ...
            Command[] setCommands)
        {
            PBS.Battle.View.Events.Pokemon pokemon = battleModel.GetMatchingPokemon(agent.pokemonUniqueID);
            ...
            Command[] setCommands)
        {
            ...
            moveslots = new List<PBS.Battle.View.Events.CommandAgent.Moveslot>(agent.moveslots);
            ...
            else
            {
                ...
                if (battleModel.battleSettings.canMegaEvolve
                    && bEvent.canMegaEvolve
                    && agent.canMegaEvolve
                    && !canZMove 
                    && !canDynamax 
                    && !canFormChange)
                ...
                // Check Z-Move
                if (battleModel.battleSettings.canZMove
                    && bEvent.canZMove
                    && agent.canZMove
                    && !canMegaEvolve 
                    && !canDynamax)
                ...
                // Check Dynamax
                if (battleModel.battleSettings.canDynamax
                    && bEvent.canDynamax
                    && agent.canDynamax
                    && !canMegaEvolve 
                    && !canZMove)
        ...
        public IEnumerator ControlPromptParty(
            ...
            PBS.Battle.View.Events.CommandGeneralPrompt bEvent = null,
            PBS.Battle.View.Events.CommandReplacementPrompt bEventR = null
            )
        {
            PBS.Battle.View.Events.Pokemon pokemon = battleModel.GetMatchingPokemon(agent.pokemonUniqueID);
            commandPokemon = pokemon;
        ...
        private void SelectCommandMenu()
        {
            ...
            else
            {
                ...
                {
                    Command attemptedCommand = Command.CreateRunCommand(commandPokemon, true);
        ...
        private void NavigateFightMenuQuad(InputAction.CallbackContext obj)
        {
            ...
            if (scrollX != 0 || scrollY != 0)
            {
                ...
                int columns = 2;
                PBS.Battle.View.Events.CommandAgent.Moveslot[,] map = GetMoveslotMap(rows, columns, accountForBack, moveslots);
                ...
                if (curPos[0] != -1)
                {
                    ...
                    PBS.Battle.View.Events.CommandAgent.Moveslot nxtSlot = map[newRow, newColumn];
                    newPos = moveslots.IndexOf(nxtSlot);
                }
                NavigateFightMenu(newPos - moveIndex);
            }
        }
        private PBS.Battle.View.Events.CommandAgent.Moveslot[,] GetMoveslotMap(int rows, int columns, bool accountForBack, List<PBS.Battle.View.Events.CommandAgent.Moveslot> list)
        {
            PBS.Battle.View.Events.CommandAgent.Moveslot[,] map = new PBS.Battle.View.Events.CommandAgent.Moveslot[rows, columns];
            int start = accountForBack ? 1 : 0;
            for (int i = 0; i < rows; i++)
            {
                for (int j = 0; j < columns; j++)
        ...
        private int[] GetMoveslotXYPos(PBS.Battle.View.Events.CommandAgent.Moveslot moveslot, PBS.Battle.View.Events.CommandAgent.Moveslot[,] map)
        {
            for (int i = 0; i < map.GetLength(0); i++)
            {
                for (int j = 0; j < map.GetLength(1); j++)
                ...
                if (validMove)
                {
                    ...
                    if (battleModel.DoesBattleAutoTarget())
                    {
                        Command attemptedCommand = Command.CreateMoveCommand(
                            commandUser: commandPokemon.uniqueID,
                ...
                else if (commandType == BattleExtraCommand.Switch)
                {
                    ...
                    Command attemptedCommand = (forceReplace) ?
                        Command.CreateReplaceCommand(
                            switchPosition,
                            commandTrainer.playerID,
                            choice,
                            true)
                        :
                        Command.CreateSwitchCommand(
                            commandPokemon.uniqueID,
                            switchPosition,
                            commandTrainer.playerID,
                            choice,
                            true);
            ...
            Action<bool> callback)
        {
            ...
            PBS.Battle.View.Compact.Pokemon userPokemon = battleModel.GetMatchingPokemon(attemptedCommand.commandUser);
            ...
            else if (attemptedCommand.commandType == BattleCommandType.Bag)
            {
                ...
                if (commandSuccess)
                {
                    ...
                    for (int i = 0; i < setCommands.Length; i++)
                    {
                        if (setCommands[i] != null)
                        {
                            Command itemCommand = setCommands[i];
